...
uint8_t handleBattleEvent(EVENT_ID eventID) {
	traceLog();
	if (!isInited || first_check || battleEnded || !g_self || eventID == EVENT_ID::IN_HANGAR || !M_MODELS_NOT_USING) {
		traceLog();
		return 1;
	} traceLog();
					...
					if (eventID == EVENT_ID::IN_BATTLE_GET_FULL) {
						...
						BEGIN_USING_MODELS;
							case WAIT_OBJECT_0: traceLog();
								...
								//освобождаем мутекс дл€ этого потока
							...
							case WAIT_ABANDONED: traceLog();
								...
								/*
								ѕервый способ - нативный вызов в main-потоке добавлением в очередь. ЌенадЄжно!
								...
								*/
								...
								/*
								¬торой способ - вызов асинхронной функции BigWorld.fetchModel(path, onLoadedMethod)

								Ѕолее-менее надежно, выполн€етс€ на уровне движка
								*/
								...
								//ожидаем событи€ полного создани€ моделей
				...
				BEGIN_USING_MODELS;
					case WAIT_OBJECT_0: traceLog();
						...
						while (it_sect_sync != sync_map.modelsSects_deleting.end()) {
							...
							it_sect_sync = sync_map.modelsSects_deleting.erase(it_sect_sync); //удал€ем секцию из вектора секций синхронизации
						} traceLog();
...
uint8_t handleBattleEndEvent(PyThreadState* _save) {
	traceLog();
	if (!isInited || first_check || !M_MODELS_NOT_USING) {
		traceLog();
		return 1;
	} traceLog();
