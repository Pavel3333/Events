...
static PyObject* event_model(char* path, float coords[3], bool isAsync=false) {
	...
	if (isAsync) {
		...
		Model = PyObject_CallMethodObjArgs(BigWorld, __fetchModel, PyString_FromString(path), partialized, NULL); //çàïóñêàåì àñèíõðîííîå äîáàâëåíèå ìîäåëè
	...
	if (coords != nullptr) {
		if (!setModelPosition(Model, coords)) { //ñòàâèì íà íóæíóþ ïîçèöèþ
			return NULL;
		}
	}
...
static PyObject* event_onModelLoaded(PyObject *self, PyObject *args) { //ïðèíèìàåò àðãóìåíòû: óêàçàòåëü íà êîîðäèíàòû è ñàìó ìîäåëü
	if (!isInited || battleEnded) {
		Py_RETURN_NONE;
	}
...
static PyObject* event_onModelCreated(PyObject *self, PyObject *args) { //ïðèíèìàåò àðãóìåíòû: óêàçàòåëü íà êîîðäèíàòû è ñàìó ìîäåëü
	...
	if (!setModelPosition(Model, coords_f)) { //ñòàâèì íà íóæíóþ ïîçèöèþ
		Py_RETURN_NONE;
	}
	...
	if (models.size() >= allModelsCreated) { //åñëè ÷èñëî ñîçäàííûõ ìîäåëåé - ñòîëüêî æå èëè áîëüøå, ÷åì íàäî
		long mlCBID = NULL;

		callback(&mlCBID, onModelLoadedPyMeth, 0.0); //ñòàâèì êîëáåê - èíèòèì ìîäåëè
	}
...
uint8_t handle_battle_event(uint8_t eventID) {
	...
	if (current_map.stageID >= 0 && current_map.stageID < STAGES_COUNT) {    //âûâîäèì ñîîáùåíèå
		if (
				...
				if (isModelsAlreadyCreated && !isModelsAlreadyInited && current_map.minimap_count && current_map.modelsSects.size()) {
					if      (eventID == EventsID.IN_BATTLE_GET_FULL) {
						...
						*/
						...
						PyThreadState *_save;        //ãëóøèì GIL, ïîêà îæèäàåì
						Py_UNBLOCK_THREADS;
...
/*
ÏÎ×ÅÌÓ ÍÅËÜÇß ÇÀÊÐÛÂÀÒÜ ÒÀÉÌÅÐÛ Â ÑÀÌÈÕ ÑÅÁÅ

-ïîòîê îòêðûë îæèäàþùèé òàéìåð
-òàéìåð è ãîâîðèò åìó: ïîòîê, ó ìåíÿ òóò àøûïêà
-òàéìåð ñäåëàë õàðàêèðè
-ïîòîê: ÒÀÉÌÅÐ!!!
-ïðîãðàììà óøëà â âå÷íîå îæèäàíèå
*/
...

{
	...
	//âûêëþ÷àåì GIL äëÿ ýòîãî ïîòîêà
...
DWORD WINAPI SecondThread(LPVOID lpParam)
{
	...
	case WAIT_OBJECT_0:
		...
		if (eventID != EventsID.IN_HANGAR) {
			ResetEvent(EVENT_IN_HANGAR->hEvent); //åñëè èâåíò íå ñîâïàë ñ íóæíûì - ÷òî-òî èäåò íå òàê, ãëóøèì òðåä, ñëåäóþùèé çàïóñê òðåäà ïðè âõîäå â àíãàð
	...
	case WAIT_OBJECT_0:
		...
		if (EVENT_START_TIMER->eventID != EventsID.IN_BATTLE_GET_FULL && EVENT_START_TIMER->eventID != EventsID.IN_BATTLE_GET_SYNC) {
			ResetEvent(EVENT_START_TIMER->hEvent); //åñëè èâåíò íå ñîâïàë ñ íóæíûì - ÷òî-òî èäåò íå òàê, ãëóøèì òðåä, ñëåäóþùèé çàïóñê òðåäà ïðè âõîäå â àíãàð
	...
	ExitThread(NULL); //çàâåðøàåì ïîòîê
...
uint8_t makeEventInThread(uint8_t map_ID, uint8_t eventID) { //ïåðåâîäèì èâåíòû â ñèãíàëüíûå ñîñòîÿíèÿ
	if (!isInited || !databaseID || battleEnded) {
		return 1U;
	}

	if (eventID == EventsID.IN_HANGAR || eventID == EventsID.IN_BATTLE_GET_FULL || eventID == EventsID.IN_BATTLE_GET_SYNC) { //ïîñûëàåì èâåíò è îáðàáàòûâàåì â òðåäå
		if      (eventID == EventsID.IN_HANGAR) {
			if (!EVENT_IN_HANGAR) {
				return 4;
			}

			EVENT_IN_HANGAR->databaseID = databaseID; //çàïîëíÿåì áóôåð äëÿ àíãàðà
...
void closeEvent1(PEVENTDATA_1* pEvent) {
	if (*pEvent != NULL) { //åñëè óæå áûëà èíèöèàëèçèðîâàíà ñòðóêòóðà - óäàëÿåì
...
static PyObject* event_fini_py(PyObject *self, PyObject *args) {
	...
	if (fini_result) {
		...
		if (hTimer != NULL) { //çàêðûâàåì òàéìåð, åñëè îí áûë ñîçäàí
			CloseHandle(hTimer);
...
bool createEvent1(PEVENTDATA_1* pEvent, uint8_t eventID) {
	closeEvent1(pEvent); //çàêðûâàåì èâåíò, åñëè ñóùåñòâóåò

	*pEvent = (PEVENTDATA_1)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, //âûäåëÿåì ïàìÿòü â êó÷å äëÿ èâåíòà
		sizeof(EVENTDATA_1));

	if (*pEvent == NULL) //íåõâàòêà ïàìÿòè, çàâåðøàåì ðàáîòó
	{
		ExitProcess(1);
	}
...
bool createEvent2(PEVENTDATA_2* pEvent, LPCWSTR eventName) {
	closeEvent2(pEvent); //çàêðûâàåì èâåíò, åñëè ñóùåñòâóåò

	*pEvent = (PEVENTDATA_2)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, //âûäåëÿåì ïàìÿòü â êó÷å äëÿ èâåíòà
		sizeof(EVENTDATA_2));
...
bool createEventsAndSecondThread() {
	...
	//TODO: ñäåëàòü èâåíò - óäàëåíèå áëèæàéøåé ìîäåëè
	...
	hSecondThread = CreateThread( //ñîçäàåì âòîðîé ïîòîê
		NULL,                                   // default security attributes
...
uint8_t event_ñheck() {
	...
	// èíèöèàëèçàöèÿ âòîðîãî ïîòîêà, åñëè íå ñóùåñòâóåò, èíà÷å - çàâåðøèòü âòîðîé ïîòîê è íà÷àòü íîâûé
...
static PyObject* event_ñheck_py(PyObject *self, PyObject *args) {
	uint8_t res = event_ñheck();
...
{
	{ "b",             event_ñheck_py,                METH_VARARGS, ":P" }, //check
